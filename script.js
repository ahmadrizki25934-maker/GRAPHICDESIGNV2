...
// OPTIMASI 1: Setel model paling dasar dan kurangi batas minimal pelacakan agar kalkulasi super instan
hands.setOptions({
    maxNumHands: 2,
    modelComplexity: 0, 
...
// OPTIMASI 2: Turunkan resolusi input tracking AI ke 640x360 (Beban kerja AI turun 4x lipat!)
// Langkah ini dijamin mendongkrak FPS di atas 20-40 FPS tanpa merusak tampilan visual luar.
...
// ===== ANTI-DELAY & HIGH-FPS LERP =====
function adaptiveLerp(current, target) {
    const distance = Math.hypot(target.x - current.x, target.y - current.y);
    
    // Lerp factor dinaikkan agar gerakan HUD terasa instan mengikuti tangan (Zero Lag)
    let lerpFactor = 0.35; 
    if (distance < 5) {
        lerpFactor = 0.12; 
    } else if (distance > 20) {
...
function onHandResults(results) {
    // Sinkronisasi resolusi visual canvas ke ukuran video asli agar tetap tajam (1280x720)
    ...
    if (results.multiHandLandmarks && results.multiHandedness) {
        results.multiHandLandmarks.forEach((landmarks, index) => {
            ...
            // OPTIMASI 3: Mengurangi ketebalan garis skeleton bawaan agar proses render canvas ringan
    ...
    if (leftHand && rightHand) {
        ...
        hudFrame.opacity = Math.min(1, hudFrame.opacity + 0.2); // Transisi pemudar dipercepat
    } else {
        hudFrame.isValid = false;
        hudFrame.opacity = Math.max(0, hudFrame.opacity - 0.15); 
    }

    // SMOOTHING & RENDER HUD
...
function renderCyberHUDFrame() {
    ...
    // OPTIMASI 4: Shadow blur dikurangi saat bergerak dinamis agar GPU tidak drop fps
